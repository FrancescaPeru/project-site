...
We chose the _Basilica di Santo Stefano_ as our first topic. We examined the ArCo ontology to see to which class the churches belong and we found two classes: a more general one (arco:ImmovableCulturalProperty) and a more specific one (arco:ArchitecturalOrLandscapeHeritage). We chose "ImmovableCulturalProperty". Then, we did the first query in order to find the _Basilica di Santo Stefano_ on ArCo, and we also employed the keywords **DISTINCT** to be sure not to have any duplicates in the results, and **FILTER** to see if the label exactly matches the string.
...
Among the churches in _Piazza Santo Stefano_ that form part of the Basilica, we chose _Chiesa del Crocifisso_ ([IRI Chiesa del Crocifisso](https://w3id.org/arco/resource/ArchitecturalOrLandscapeHeritage/0800242970)), and by analyzing its RDF page, we noticed that among the missing information there is the saint to whom it is dedicated.
...
![Step4](assets/images/Step4-pt2.png)
...
<https://w3id.org/arco/resource/HistoricOrArtisticProperty/0800018958> 
a-cd:isLocatedIn 
<https://w3id.org/arco/resource/ArchitecturalOrLandscapeHeritage/0800108309> 
